...
const formattedPath = computed(() =>{
  ...
  if (fullPath.includes('projects') && name !== 'project') return fullPath.split('/')[2].split('-').join(' ')[0].toUpperCase() + fullPath.split('/')[2].split('-').join(' ').slice(1)
...
<template>
  <div class="absolute w-full flex items-center justify-between py-4 z-10 transition duration-150">
	...
	<Transition name="slide">
		<div v-if="sidebar" class="fixed z-30 w-full px-2 flex flex-col md:hidden">
			<div class="w-full flex justify-between mt-4 items-center font-semibold text-lg pr-full bg-zinc-300 dark:bg-zinc-800 py-2 rounded-t-lg pl-4 shadow">
				...
				<button type="button" @click="sidebar = false" aria-label="Sidebar" class="mr-4 p-1.5 bg-[#b4b4b8] dark:bg-zinc-700 dark:text-white dark:hover:bg-zinc-900 hover:bg-zinc-400 focus:ring-[3px] focus:ring-indigo-400 outline-none rounded duration-200">
			...
			<div class="flex flex-col font-mont font-semibold w-full bg-zinc-300 pb-4 rounded-b-lg dark:bg-zinc-800 dark:text-zinc-50">
				...
				<NuxtLink class="mx-3 py-1.5 px-2.5 text-indigo-500 rounded hover:bg-[#b4b4b8] dark:hover:bg-zinc-900 duration-150">Log in</NuxtLink>
				<NuxtLink class="mx-3 py-1.5 px-2.5 text-indigo-500 rounded hover:bg-[#b4b4b8] dark:hover:bg-zinc-900 duration-150">Register</NuxtLink>
			</div>
		</div>
	</Transition>
  <div v-if="sidebar" @click="sidebar = false" class="h-full w-full fixed z-10"></div>
</template>
