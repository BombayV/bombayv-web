...
const setSidebar = (value?: boolean) => {
  activeSidebar.value = value || !activeSidebar.value
...
<template>
  <div>
    <header ref="navbarRef" class="navbar fixed transition-transform transform-gpu duration-500 z-20">
      ...
      <div class="flex-none hidden px-2 mx-2 lg:flex">
        <div class="flex items-stretch">
          ...
          <button @click="auth.signOut" v-if="user" class="btn btn-ghost btn-sm rounded-btn xl:text-md hover:bg-error/80 hover:text-white">Log out</button>
        </div>
      </div>
      <div @click="setSidebar" class="btn lg:hidden btn-square btn-ghost">
