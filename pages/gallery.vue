...
<template>
  <div v-if="!loading" class="h-auto bg-zinc-200 pb-8 dark:bg-zinc-900 duration-150">
    ...
    <div v-if="pics.length !== 0" class="px-16 pt-6 pb-2 columns-xs 2xl:columns-sm gap-6" :class="activeData.url !== '' && 'blur'">
      <img v-for="image in activePics" @click="activeData = { title: image.title, url: image.url }" class="cursor-pointer mx-auto hover:opacity-90 duration-150 transition-opacity rounded-md img-shadow border border-8 border-white dark:border-zinc-800 mt-6 duration-150" :src="image.url" loading="lazy" alt="Loading image...">
    </div>
