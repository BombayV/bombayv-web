  ...
  <div>
    <NuxtLayout name="authnavbar">
      <div class="relative flex flex-col py-20 min-h-screen w-full px-4 max-w-7xl mx-auto">
        <div
          class="h-20 bg-base-300 w-full rounded-full relative flex items-center justify-between px-4 shadow-md drop-shadow-md">
          <div class="flex items-center gap-x-4">
            <NuxtImg
              :src="userData.avatarUrl"
              class="rounded-full w-16 h-16 border-2 border-blue-300" />
          ...
          <div class="flex gap-x-2">
            <button
              type="button"
              class="bg-info/60 hover:bg-info/70 rounded-full grid place-items-center btn btn-circle">
              <svg
                ...
                stroke-width="1.5"
                stroke="currentColor"
                class="w-6 h-6 fill-primary-content/80">
                <path
            ...
            <button
              @click="userData.signOut(auth, navigateTo)"
              type="button"
              class="bg-error/60 hover:bg-error/70 rounded-full grid place-items-center btn btn-circle">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
                class="w-6 h-6 fill-primary-content/60">
                <path
                  fill-rule="evenodd"
        ...
        <div class="grid grid-rows-2 md:grid-cols-2 md:grid-rows-none grow mt-8 gap-y-4 md:gap-x-8">
          <div class="bg-base-300 rounded-xl min-h-[20rem] px-4 py-4 flex flex-col justify-center">
            ...
            <div class="grid grow" :class="blogPosts.length > 0 ? '' : 'place-items-center'">
              <p v-if="blogPosts.length < 1" class="text-4xl font-bold text-primary-content/60">
                No Blog Posts
              </p>
            </div>
          </div>
          <div class="bg-base-300 rounded-xl min-h-[20rem] px-4 py-4 flex flex-col justify-center">
            ...
            <div class="grid grow" :class="images.length > 0 ? '' : 'place-items-center'">
              <p v-if="images.length < 1" class="text-4xl font-bold text-primary-content/60">
                No Images
              </p>
            </div>
          </div>
